...
impl WeaponFactory for MeleeWeaponFactory {
    fn create_weapon(&self) -> Box<dyn Weapon> {
        Box::new(Sword::new(
            ...
            random_rarity(),
            random_stat_int_within_range(50, 200),
            random_stat_int_within_range(5, 15),
            random_stat_within_range(10.0, 50.0),
            random_stat_within_range(1.0, 2.0),
            1,
            0,
        ))
    }
...
impl WeaponFactory for RangedWeaponFactory {
    fn create_weapon(&self) -> Box<dyn Weapon> {
        Box::new(Bow::new(
            ...
            random_rarity(),
            random_stat_int_within_range(100, 255),
            random_stat_int_within_range(5, 10),
            random_stat_within_range(20.0, 100.0),
            random_stat_within_range(1.0, 2.5),
            random_stat_int_within_range(20, 100),
            ArrowType::Fire,
            0,
        ))
    }
...
impl WeaponFactory for SpearWeaponFactory {
    fn create_weapon(&self) -> Box<dyn Weapon> {
        Box::new(Spear::new(
            ...
            random_rarity(),
            random_stat_int_within_range(100, 255),
            random_stat_int_within_range(6, 12),
            random_stat_within_range(15.0, 75.0),
            random_stat_within_range(1.2, 2.2),
            random_stat_int_within_range(10, 50),
            random_stat_int_within_range(10, 30),
            0,
        ))
    }
}
...
lazy_static! {
    static ref WEAPON_FACTORIES: RwLock<HashMap<&'static str, Box<dyn WeaponFactory + Send + Sync>>> = {
        ...
        factories.insert("Sword", Box::new(MeleeWeaponFactory));
        factories.insert("Bow", Box::new(RangedWeaponFactory));
        factories.insert("Spear", Box::new(SpearWeaponFactory));
        RwLock::new(factories)
    };
    static ref WEAPON_ARMORY: RwLock<HashMap<&'static str, Box<dyn Weapon + Send + Sync>>> = {
        let armory = HashMap::<&'static str, Box<dyn Weapon + Send + Sync>>::new();
        RwLock::new(armory)
    };
}
...
impl RandomWeaponFactory {
    pub async fn create_weapons(
        &self,
        count: usize,
        weapon_selector: Option<(&str, u8)>,
    ) -> Vec<Box<dyn Weapon>> {
...
mod test {
    ...
    async fn test_specific_weapon_generation() {
        ...
        for weapon in bows.iter() {
            assert!(
                weapon.as_any().downcast_ref::<Bow>().is_some(),
                "Bow bekleniyordu."
            );
        }
        for weapon in spears.iter() {
            assert!(
                weapon.as_any().downcast_ref::<Spear>().is_some(),
                "Spear bekleniyordu."
            );
        }
    }
    ...
    async fn get_min_6_sword_from_stack() -> Vec<Box<dyn Weapon>> {
        ...
        weapons
            .into_iter()
            .filter(|weapon| weapon.as_any().downcast_ref::<Sword>().is_some())
            .collect()
    }
